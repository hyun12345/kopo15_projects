...
<script language="JavaScript">
 ...
 function checkSpecial(str) {
    ...
    var filecheck1 = gongji_reinsert.file1;
    var filecheck2 = gongji_reinsert.file2;
    var filecheck3 = gongji_reinsert.file3;
        ...
        gongji_reinsert.content.value = $('#content').summernote('code');
        gongji_reinsert.action = "gongji_write.jsp?key=REINSERT";
...
String k15_Query = "select date_format(date, '%Y-%m-%d %H:%i:%s'), rootid, relevel, recnt from gongji where id = '" + key + "';";
ResultSet k15_rset = k15_stmt.executeQuery(k15_Query);

k15_rset.next();
String date = k15_rset.getString(1);
int rootid = k15_rset.getInt(2);
int relevel = k15_rset.getInt(3);
int recnt = k15_rset.getInt(4);
%>

<form method=post name=gongji_reinsert enctype="multipart/form-data">
<%-- form의 이름을 gongji_write로 설정 --%>
<table width=700 border=1 cellspacing=0 cellpadding=5 align=center>
<tr>
<td align=center><b>번호</b></td>
<td colspan=2>댓글(insert)<input type=hidden name=key value="REINSERT" readonly></td>
<%-- 번호(id)의 경우 auto_increment 설정이 되어있으므로 입력불가능(readonly) 설정 뒤 value는 INSERT를 갖도록 설정 --%>
<%-- type=hidden : 사용자에게 보이지 않지만 서버로 넘어가는 필드 --%>
</tr>
<tr>
<td align=center><b>제목</b></td>
<td colspan=2><input type=text name=title size=70 maxlength=70 placeholder="제목을 입력하십시오." required></td>
<%-- type=text : 텍스트 형태로 입력 받음 / size=70 : 폭 70px / 최대 문자 길이 : maxlength / 필수입력사항(required) --%>
</tr>
<tr>
<td align=center><b>일자</b></td>
<%
out.println("<td colspan=2>" + date + "<input type=hidden name=date value=" + date + " readonly></td>");
%>
</tr>
<tr>
<td align=center><b>내용</b></td>
<%-- <td colspan=2><textarea id=content name=content cols=70 row=600 placeholder="내용을 입력하십시오." required></textarea></td> --%>
<td><div id=content></div>
<input type=hidden name=content value=""></td>
<%-- textarea의 넓이를 500, 높이를 250으로 설정 / cols : 가로 텍스트 수 & rows : 세로 텍스트 수 --%>
</tr>
<tr>
<td align=center><b>원글</b></td>
<%
out.println("<td colspan=2>"+ rootid + "<input type=hidden value='" + rootid + "' name=rootid readonly></td>");
%>
</tr>
<tr>
<td align=center><b>파일 첨부</b></td>
<td colspan=2>
<form method=post action=twice_write.jsp >
...
<tr>
<td align=center><b>댓글수준</b></td>
<td><input type=number name=relevel value='<%= relevel +1 %>'size=20 readonly>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
<b>댓글내 순서</b>
<%
...
if(relevel != 0){
    k15_rset = k15_stmt.executeQuery("select recnt, id from gongji where rootid = " + rootid + " and relevel=" + relevel + " order by recnt asc;");
    while(k15_rset.next()){
        if(rootid == Integer.parseInt(key)){
            break;
        }
    }
    if(k15_rset.next()){
        out.println("<input type=number name=recnt size=20 readonly value="+ k15_rset.getInt(1) +"></td>");
    }else{
        k15_rset = k15_stmt.executeQuery("select max(recnt)+1 from gongji where rootid = " + rootid +" and relevel >= "+ relevel +" and recnt >= "+ recnt +";");
        k15_rset.next();
        out.println("<input type=number name=recnt size=20 readonly value="+ k15_rset.getInt(1) +"></td>");
    }
}else{
    k15_rset = k15_stmt.executeQuery("select max(recnt)+1 from gongji where rootid = "+ rootid +";");
    k15_rset.next();
    out.println("<input type=number name=recnt size=20 readonly value=" +k15_rset.getInt(1) +"></td>");
}
%>
